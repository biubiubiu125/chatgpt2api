    ...
    <div ref="scrollEl" class="studio-chat-scroll custom-scrollbar" @scroll="handleScroll">
      ...
      <div v-else ref="turnsEl" class="studio-turns">
        ...
        <StudioMessageItem
          ...
          @image-asset-load="forwardImageAssetLoad"
          @image-asset-error="forwardImageAssetError"
        />
      </div>
    </div>
      ...
      >
        ...
        <div class="studio-search-drawer-body custom-scrollbar">
          <a
            ...
            :class="{ 'is-static': !safeSearchSourceHref(source.url), 'is-highlighted': highlightedSearchSourceId === searchSourceDomId(activeSearchSourceMessage.id, sourceIndex) }"
            :href="safeSearchSourceHref(source.url) || undefined"
            :target="safeSearchSourceHref(source.url) ? '_blank' : undefined"
            rel="noopener noreferrer"
            @click="!safeSearchSourceHref(source.url) && $event.preventDefault()"
          >
            ...
            <Icon v-if="safeSearchSourceHref(source.url)" icon="lucide:external-link" class="studio-search-source-open h-3.5 w-3.5" />
          </a>
        </div>
      </aside>
    </Transition>
...
import { imageTaskMessageLookupId, imageTaskVisiblePrimaryMessage } from '@/lib/imageTaskDelivery'
import { splitStudioSearchCalls } from '@/lib/studioToolCalls'
import { safeExternalHost, safeExternalHref } from '@/utils/safeExternalUrl'
...
const emit = defineEmits<{
  ...
  'image-asset-load': [message: StudioMessage, asset: StudioImageAssetView]
  'image-asset-error': [message: StudioMessage, asset: StudioImageAssetView]
}>()
...
function buildMessageView(message: StudioMessage): StudioMessageView {
  ...
  const task = lookupId ? props.taskById.get(lookupId) : undefined
  ...
  const isPendingImageMessage = isImageMessage && (!task || !isTerminalImageTaskStatus(task.status))
  ...
  const view: StudioMessageView = {
    ...
    primaryMessage: imageTaskVisiblePrimaryMessage(message, task, taskPrimaryMessage(task)),
...
function countRenderableImageAssets(assets: readonly ImageTaskAsset[]) {
  let count = 0
  for (const asset of assets) {
    if (imageAssetUrl(asset)) count += 1
  }
  return count
}
...
function isLiveMessage(message: StudioMessage) {
  ...
  const task = props.taskById.get(lookupId)
  if (!task) return false
  if (!isTerminalImageTaskStatus(task.status)) return true
  return isUnacknowledgedRenderableImageTask(task)
}

function isTerminalImageTaskStatus(status: string | undefined) {
  return status === 'success' || status === 'failed' || status === 'canceled'
}

function isUnacknowledgedRenderableImageTask(task: ImageTask) {
  if (task.delivery_status === 'acknowledged') return false
  const hasResult = task.status === 'success'
    || ((task.status === 'failed' || task.status === 'canceled') && Number(task.succeeded_jobs || 0) > 0)
  if (!hasResult) return false
  return countRenderableImageAssets(task.data || EMPTY_IMAGE_ASSETS) > 0
}
...
function sourceHost(url: string | undefined) {
  return safeExternalHost(url)
}

function safeSearchSourceHref(url: string | undefined) {
...
function forwardImageAssetLoad(message: StudioMessage, asset: StudioImageAssetView) {
  emit('image-asset-load', message, asset)
}

function forwardImageAssetError(message: StudioMessage, asset: StudioImageAssetView) {
  emit('image-asset-error', message, asset)
}
