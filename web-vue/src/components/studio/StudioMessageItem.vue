      ...
      <div class="chat-message-bubble-wrap">
        ...
        >
          ...
          >
            ...
            <template v-else>
              <div v-if="message.status === 'error' && !message.assets.length" class="studio-image-status is-error">
              ...
              >
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        loading="lazy"
                        @load="$emit('image-asset-load', message, asset)"
                        @error="$emit('image-asset-error', message, asset)"
                      />
                ...
                <div
                  v-if="message.status === 'error' && message.assets.length"
                  class="studio-image-status is-error studio-image-partial-error"
                >
                  <Icon icon="lucide:circle-alert" class="h-4 w-4" />
                  <span>{{ message.primaryMessage || '部分图片生成失败，已保留成功生成的图片。' }}</span>
                </div>
              </div>
            </template>
          </div>
