...
const DEFAULT_PROXY_RUNTIME_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36'

function chrome146UserAgent(value: unknown): string {
  void value
  return DEFAULT_PROXY_RUNTIME_USER_AGENT
}
...
export function normalizeProxyRuntime(raw: unknown): ProxyRuntimeSettings {
  ...
  return {
    ...
    clearance: {
      ...
      user_agent: chrome146UserAgent(clearance.user_agent),
...
export function normalizeSettings(raw: RawSettings | null | undefined): Settings {
  ...
  const normalized = {
    ...
    image_preflight_token_refresh_enabled: boolValue(source.image_preflight_token_refresh_enabled, false),
    image_upscale_enabled: boolValue(source.image_upscale_enabled, false),
    image_upscale_engine: source.image_upscale_engine === 'pillow_lanczos' ? 'pillow_lanczos' : 'sharp_lanczos3',
    image_auth_refresh_concurrency: numberValue(source.image_auth_refresh_concurrency, 10, 1),
    image_max_account_attempts: numberValue(source.image_max_account_attempts, 4, 2),
    ...
    runtime_capacity: {
      uvicorn_workers: numberValue(source.runtime_capacity?.uvicorn_workers, 1, 1),
      text_concurrency_limit: numberValue(source.runtime_capacity?.text_concurrency_limit, 80, 1),
      image_concurrency_limit: numberValue(source.runtime_capacity?.image_concurrency_limit, 2000, 1),
      request_queue_timeout_seconds: numberValue(source.runtime_capacity?.request_queue_timeout_seconds, 0, 0),
    },
    image_storage: {
      ...
      private_webdav_url: cleanString(imageStorage.private_webdav_url),
      private_webdav_username: cleanString(imageStorage.private_webdav_username),
      private_webdav_password: cleanString(imageStorage.private_webdav_password),
      private_webdav_root_path: cleanString(imageStorage.private_webdav_root_path) || 'chatgpt2api/private',
    ...
    backup: {
      ...
      encrypt: boolValue(backup.encrypt, true),
      ...
      include: {
        ...
        images: boolValue(backupInclude.images, true),
      },
    },
    third_party_apps: {
      infinite_canvas: {
...
function toBackendSettings(settings: Settings): RawSettings {
  ...
  const payload: RawSettings = {
    ...
    image_preflight_token_refresh_enabled: boolValue(normalized.image_preflight_token_refresh_enabled, false),
    image_upscale_enabled: boolValue(normalized.image_upscale_enabled, false),
    image_upscale_engine: normalized.image_upscale_engine === 'pillow_lanczos' ? 'pillow_lanczos' : 'sharp_lanczos3',
    image_auth_refresh_concurrency: numberValue(normalized.image_auth_refresh_concurrency, 10, 1),
    image_max_account_attempts: numberValue(normalized.image_max_account_attempts, 4, 2),
