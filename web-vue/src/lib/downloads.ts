...
export async function downloadUrlAsFile(
  url: string,
  filename: string,
  options: { localPath?: string; authorization?: boolean } = {},
) {
  const value = cleanString(url)
  ...
  try {
    const authorization = (options.authorization === true || shouldAuthorizeDownloadUrl(value))
      && isTrustedApiDownloadUrl(value, { apiBaseUrl })
    const blob = await fetchBlob(value, authorization)
