  ...
  <div class="studio-workspace" :class="{ 'is-fullscreen': isFullscreen }" :style="workspaceStyle">
    ...
    <main class="studio-main">
      ...
      <StudioMessageList
        ...
        @image-asset-load="handleImageAssetLoad"
        @image-asset-error="handleImageAssetError"
      />
...
import { IMAGE_LOAD_ERROR_PREFIX, resolveImageTaskAssetFetchSource } from '@/lib/imageTaskDelivery'
import {
...
function handleImageAssetLoad(message: StudioMessage, asset: StudioImageAssetView) {
  if (!message.taskId) return
  void imageTaskRuntime.acknowledgeRenderedImageTask(message.taskId, asset.path || asset.url)
}

function handleImageAssetError(message: StudioMessage, asset: StudioImageAssetView) {
  const assetKey = asset.path || asset.url || 'image'
  const error = `${IMAGE_LOAD_ERROR_PREFIX}${assetKey ? ` (${assetKey})` : ''}`
  const patched = messageRuntime.patchMessage(message.id, {
    status: 'error',
    error,
    content: message.content || error,
  })
  if (patched) {
    markConversationNotice(patched.conversation.id, 'error')
  }
}
...
function imageAssetSource(asset: StudioImageAssetView) {
  return resolveImageTaskAssetFetchSource(asset)
}
...
async function submitInpaintEdit(payload: { prompt: string; markedImage: File }) {
  ...
  let sourceFile: File
  let sourceReferenceImage: StudioReferenceImage
  let markedReferenceImage: StudioReferenceImage
  try {
    sourceFile = await imageAssetToFile(target.asset, target.name)
    sourceReferenceImage = await messageReferenceImageFromFile(sourceFile)
    markedReferenceImage = await messageReferenceImageFromFile(payload.markedImage)
  } catch (error) {
  ...
  const ok = await sendImageEditRequest({
    ...
    referenceImages: [sourceReferenceImage, markedReferenceImage],
