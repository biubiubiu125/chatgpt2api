...
export function safeStudioSearchSourceUrl(value: unknown) {
  ...
  const sources = value
    .map((item): StudioSearchSource | null => {
      ...
      const source = {
        ...
        url: safeStudioSearchSourceUrl(raw.url),
...
function parseStudioLegacySearchSourceLine(line: string): StudioSearchSource | null {
  ...
  const url = safeStudioSearchSourceUrl(cleanStudioText(match[2]).replace(/%20/g, ' ').replace(/%29/g, ')'))
