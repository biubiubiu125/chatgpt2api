...
function dataUrlBytes(dataUrl: string) {
  ...
  const binary = /;base64/i.test(meta)
    ? atob(payload)
...
export function fileFromStudioReferenceImage(reference: StudioReferenceImage, fallbackIndex = 0): File | null {
  if (typeof File === 'undefined') return null
  try {
    const decoded = dataUrlBytes(String(reference.dataUrl || ''))
    if (!decoded) return null
    const type = reference.type || decoded.mime || 'image/png'
    const extension = type.split('/')[1] || 'png'
    const name = reference.name || `reference-${fallbackIndex + 1}.${extension}`
    return new File([decoded.bytes], name, { type })
  } catch {
    return null
  }
}

export function filesFromStudioReferenceImages(
  images: StudioReferenceImage[] | undefined,
  maxFiles = DEFAULT_MAX_REFERENCE_FILES,
) {
  return (images || [])
    .slice(0, maxFiles)
    .map((reference, index) => fileFromStudioReferenceImage(reference, index))
...
export function useStudioReferenceRuntime(options: StudioReferenceRuntimeOptions = {}) {
  ...
  function restoreFromMessageReferenceImages(images: StudioReferenceImage[] | undefined) {
    const restored = (images || [])
      .slice(0, maxFiles)
      .map((reference, index) => ({
        reference,
        file: fileFromStudioReferenceImage(reference, index),
      }))
      .filter((item): item is { reference: StudioReferenceImage; file: File } => Boolean(item.file))
    files.value = restored.map((item) => item.file)
    references.value = restored
      .map(({ reference, file }) => ({
        id: reference.id || createStudioId('source'),
        name: reference.name || file.name || '参考图',
        type: reference.type || file.type || 'image/png',
        size: reference.size || file.size || 0,
        dataUrl: reference.dataUrl,
        previewDataUrl: reference.dataUrl,
      }))
  ...
  return {
    ...
    remove,
    clear,
    restoreFromMessageReferenceImages,
    open,
    openPreview,
    closePreview,
  }
