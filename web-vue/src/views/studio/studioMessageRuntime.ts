...
export function useStudioMessageRuntime(input: StudioMessageRuntimeInput) {
  ...
  function patchMessage(messageId: string, patch: Partial<StudioMessage>) {
    ...
    target.conversation.messages = [
      ...target.conversation.messages.slice(0, target.index),
      next,
      ...target.conversation.messages.slice(target.index + 1),
    ]
