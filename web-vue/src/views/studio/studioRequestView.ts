...
export async function createStudioImageTask(input: StudioImageTaskInput): Promise<ImageTask> {
  const model = normalizeImageModel(input.imageForm.model || DEFAULT_IMAGE_MODEL)
  ...
  if (input.requiresEdit && !input.files.length) {
    throw new Error('image edit references are missing')
  }
