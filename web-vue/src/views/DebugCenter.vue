        ...
        <div class="debug-task-list">
          ...
          >
            ...
            <div v-if="task.result?.primary_url || task.result?.zip_url" class="mt-3 flex flex-wrap gap-2">
              <Button v-if="task.result?.primary_url" size="xs" variant="outline" @click="downloadFileUrl(task.result.primary_url)">下载结果</Button>
              <Button v-if="task.result?.zip_url" size="xs" variant="outline" @click="downloadFileUrl(task.result.zip_url)">下载 ZIP</Button>
            </div>
          </article>
        </div>
      </div>
...
import type { DebugChatCompletion, DebugChatMessage, DebugEditableFileTask, DebugEditableKind, DebugSearchResult, DebugSearchSource } from '@/api/debug'
import { getAuthToken } from '@/api/client'
import { useToast } from '@/composables/useToast'
import { filenameFromDownloadUrl } from '@/lib/downloadTargets'
import { downloadUrlAsFile } from '@/lib/downloads'
import { EDITABLE_FILE_TASK_REFRESH_DELAY_MS, shouldRefreshEditableFileTask } from '@/lib/editableFileTaskPolling'
...
let editableRefreshTimer: number | null = null
let editableRefreshRequestId = 0
let editableRefreshDisposed = false
...
watch(activeTab, (tab) => {
  clearEditableRefreshTimer()
  invalidateEditableRefreshRequest()
  if (tab === 'ppt') {
...
watch(editableTasks, () => scheduleEditableRefresh())

onBeforeUnmount(() => {
  clearEditableRefreshTimer()
  editableRefreshDisposed = true
  invalidateEditableRefreshRequest()
})
...
async function runSearch() {
  ...
  try {
    searchResult.value = normalizeDebugSearchResult(await debugApi.search(prompt))
  } catch (error: any) {
...
function normalizeDebugSearchResult(result: DebugSearchResult): DebugSearchResult {
  const sources = Array.isArray(result.sources)
    ? result.sources
      .map(normalizeDebugSearchSource)
      .filter((source): source is DebugSearchSource => Boolean(source))
    : undefined
  return {
    ...result,
    sources,
...
function normalizeDebugSearchSource(source: DebugSearchSource): DebugSearchSource | null {
  if (!source || typeof source !== 'object') return null
  const normalized = {
    ...source,
    url: safeExternalHref(source.url),
  }
  return normalized.title || normalized.url || normalized.snippet ? normalized : null
}
...
function isEditableTab(tab = activeTab.value) {
  return tab === 'ppt' || tab === 'psd'
}

function clearEditableRefreshTimer() {
  if (editableRefreshTimer !== null) {
    window.clearTimeout(editableRefreshTimer)
    editableRefreshTimer = null
  }
}

function invalidateEditableRefreshRequest() {
  editableRefreshRequestId += 1
  editableFetching.value = false
}

function scheduleEditableRefresh() {
  clearEditableRefreshTimer()
  if (!isEditableTab() || editableRefreshDisposed) return
  if (!editableTasks.value.some((task) => shouldRefreshEditableFileTask(task, currentEditableKind.value))) return
  editableRefreshTimer = window.setTimeout(() => {
    editableRefreshTimer = null
    void refreshEditableTasks()
  }, EDITABLE_FILE_TASK_REFRESH_DELAY_MS)
}

async function refreshEditableTasks() {
  if (editableFetching.value || editableRefreshDisposed) return
  const requestId = ++editableRefreshRequestId
  const requestKind = currentEditableKind.value
  editableFetching.value = true
  editableError.value = ''
  try {
    const response = await debugApi.listEditableFileTasks()
    if (requestId !== editableRefreshRequestId || requestKind !== currentEditableKind.value || editableRefreshDisposed) return
    editableTasks.value = (response.items || []).filter((task) => task.kind === requestKind).slice(0, 20)
  } catch (error: any) {
    if (requestId !== editableRefreshRequestId || requestKind !== currentEditableKind.value || editableRefreshDisposed) return
    editableError.value = error?.message || '刷新失败'
  } finally {
    if (requestId === editableRefreshRequestId && !editableRefreshDisposed) {
      editableFetching.value = false
...
async function submitEditableTask() {
  ...
  try {
    ...
    if (editableRefreshDisposed) return
    ...
    if (!editableRefreshDisposed) editableSubmitting.value = false
  }
}

async function downloadFileUrl(url: string) {
  try {
    await downloadUrlAsFile(url, filenameFromDownloadUrl(url), { authorization: true })
    toast.success('已开始下载')
  } catch (error: any) {
    toast.error(`下载失败：${error?.message || '无法读取文件'}`)
  }
}
</script>
