...
export const providerTypeOptions = [
  ...
  { value: 'icloud_api', label: 'iCloud 外部取码' },
...
export const providerSecretKeys = ['api_key', 'admin_key', 'admin_password', 'ddg_token', 'cf_inbox_jwt', 'cf_api_key'] as const
export const providerMailUserAgentChrome146 = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36'
...
export function providerWithTypeDraft(current: RegisterProvider, type: string): RegisterProvider {
  ...
  for (const key of providerKeysForType(type, true)) {
    ...
    if (current[key] !== undefined) {
      ...
      if (typeChanged && type === 'remail' && key === 'api_base') {
        next[key] = defaults[key]
        continue
      }
...
export function providerUsesApiBase(provider: RegisterProvider) {
  return ['cloudmail_gen', 'cloudflare_temp_email', 'moemail', 'inbucket', 'yyds_mail', 'ddg_mail', 'icloud_api', 'remail', 'donemail'].includes(providerType(provider))
}
...
export function providerUsesApiKey(provider: RegisterProvider) {
  return ['tempmail_lol', 'moemail', 'duckmail', 'gptmail', 'yyds_mail', 'icloud_api', 'remail'].includes(providerType(provider))
}
...
export function apiBaseLabel(provider: RegisterProvider) {
  ...
  if (type === 'icloud_api') return '服务地址'
...
export function registerRuntimeHint(
  ...
  if (registerTaskState(config) === 'paused') return '任务已开启但未实际运行，通常是队列或注册池暂不可用。'
