<template>
  <div class="settings-storage-grid">
    <FormSection title="图片存储" class="settings-storage-main">
      ...
      <FormField label="私有 WebDAV URL">
        <Input v-model.trim="imageStorage.private_webdav_url" block placeholder="https://example.com/private-dav" />
      </FormField>

      <div class="grid grid-cols-1 gap-2.5 md:grid-cols-2">
        <FormField label="私有用户名">
          <Input v-model.trim="imageStorage.private_webdav_username" block />
        </FormField>

        <FormField label="私有密码">
          <Input v-model="imageStorage.private_webdav_password" type="password" block />
        </FormField>
      </div>

      <FormField label="私有根路径">
        <Input v-model.trim="imageStorage.private_webdav_root_path" block placeholder="chatgpt2api/private" />
      </FormField>
