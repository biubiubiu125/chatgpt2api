<template>
  <div class="space-y-4">
    <FormSection
      v-if="mode === 'canvas'"
      title="画布入口"
      subtitle="开启后顶部导航会显示无限画布入口，并自动带上 OpenAI 兼容接口地址，不会附带调用密钥。"
    >
...
const EXAMPLE_API_KEY = '<API_KEY>'
const accessItems = computed(() => [
  { label: '服务地址', value: serviceBaseUrl.value },
  { label: 'Base URL（OpenAI）', value: openAIBaseUrl.value },
  { label: 'API Key', value: EXAMPLE_API_KEY },
  { label: '请求头', value: `Authorization: Bearer ${EXAMPLE_API_KEY}` },
])
const apiDocItems = computed(() => (
  props.mode === 'api-docs'
    ? buildApiDocItems(serviceBaseUrl.value, EXAMPLE_API_KEY)
    : []
))
</script>
