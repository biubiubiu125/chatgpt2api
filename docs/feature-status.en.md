...
| `GET /v1/models` 接口 | ✅  | 外部图片模型只暴露 `gpt-image-2`；文本模型仍返回 `auto`、`gpt-5` 系列等。内部 Codex 路由不再作为公开模型列出。 |
| 同时生成多张图片 | ✅  | 已支持，后端与前端都可进行多图生成。 |
| 图片并行生成 | ✅  | 外部生图统一进入 PostgreSQL 持久队列，按 Job 拆分并由进程内 Worker 有界线程池执行，不再按 `n` 动态开线程。 |
