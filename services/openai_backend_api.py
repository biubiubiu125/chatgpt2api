...
class OpenAIBackendAPI:
    ...
    def _build_fp(self) -> Dict[str, str]:
        ...
        fp = (
            {str(k).lower().replace("_", "-"): str(v) for k, v in raw_fp.items()}
    ...
    def _signed_asset_headers(self, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        headers = dict(extra or {})
        headers["Accept-Language"] = self.fp["accept-language"]
        return headers
        ...
        for url in urls:
            ...
            for attempt in range(2):
                ...
                def receive_chunk(chunk: bytes) -> int:
                    nonlocal download_too_large
                    if len(content_buffer) + len(chunk) > MAX_IMAGE_DOWNLOAD_BYTES:
    ...
    def stream_conversation(
        ...
        stream_timeout = config.text_stream_timeout_secs
        response = self.session.post(
            self.base_url + path,
        ...
        ensure_ok(response, path)
        try:
            yield from iter_sse_payloads(response, max_duration_secs=stream_timeout)
        finally:
            response.close()
    ...
    def _stream_picture_conversation(
        ...
        try:
            for payload in self._iter_timed_sse_payloads(
                ...
                cancel_callback=getattr(self, "cancel_callback", None),
            ):
                yield payload
