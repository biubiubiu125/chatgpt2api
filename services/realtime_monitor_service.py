...
class RealtimeMonitorService:
    def __init__(self) -> None:
        ...
        self._image_queue_provider: Any = None
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        if callable(self._image_queue_provider):
            try:
                value = self._image_queue_provider()
                image_queue = dict(value) if isinstance(value, dict) else {}
            except Exception as exc:
                image_queue = {"unavailable": True, "error": _trim(exc)}
        call_summary = self._summary(active, completed)
        queue_tasks = image_queue.get("tasks") if isinstance(image_queue.get("tasks"), dict) else {}
        queue_summary = {
            "source": "postgresql" if image_queue and not image_queue.get("unavailable") else "unavailable",
            "task_status_counts": dict(queue_tasks) if queue_tasks else {
                key: int(image_queue.get(key) or 0)
                for key in ("queued", "running", "saving", "retrying", "success", "failed", "canceled")
                if key in image_queue
            },
            "job_status_counts": dict(image_queue.get("jobs") or {}),
            "job_stage_counts": dict(image_queue.get("job_stages") or {}),
            "oldest_queued_at": image_queue.get("oldest_queued_at"),
            "queue_wait_p90_seconds": image_queue.get("queue_wait_p90_seconds"),
            "duration_p90_seconds": image_queue.get("duration_p90_seconds"),
            "active_leases": image_queue.get("active_leases"),
            "unacknowledged_success": image_queue.get("unacknowledged_success"),
            "workers": list(image_queue.get("workers") or []),
            "pause_reason": next(
                (
                    str(item.get("pause_reason") or "")
                    for item in (image_queue.get("workers") or [])
                    if isinstance(item, dict) and item.get("pause_reason")
                ),
                "",
            ),
        }
        return {
            ...
            "window": {
                "label": "结束窗口",
                "description": "最近保留的调用完成窗口，不是累计总量或当日总量",
                "source": "memory_call_window",
            ...
            "summary": {
                **call_summary,
                "image_queue": queue_summary,
                "unacknowledged_success": queue_summary.get("unacknowledged_success"),
                "completed_window_note": "call-level recent window only; durable queue stats come from image_queue",
            },
