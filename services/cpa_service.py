...
class CPAImportService:
    ...
    def _run_import(self, pool_id: str, pool: dict, names: list[str]) -> None:
        ...
        try:
            ...
            self._append_error(pool_id, "local_account_save", str(exc) or "local account save failed")
            current = self._config.get_import_job(pool_id) or {}
            self._update_job(
                pool_id,
                status="failed",
                completed=int(current.get("total") or len(names)),
        ...
        try:
            ...
            self._append_error(pool_id, "account_refresh", str(exc) or "account refresh failed")
            current = self._config.get_import_job(pool_id) or {}
            self._update_job(
                pool_id,
                status="failed",
