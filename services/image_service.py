...
def _queue_public_artifact_path(relative_path: str) -> Path | None:
    ...
    try:
        if callable(check_artifact):
            ...
            public_paths = {
                _safe_relative_path(str(item.get("path") or item.get("rel") or ""))
                for item in list_artifacts()
                if isinstance(item, dict)
            }
            public = rel in public_paths
    except Exception:
        return None
    if not public:
        return None
...
def _local_public_image_path(relative_path: str) -> Path | None:
    if image_storage_service.has_local(relative_path):
        return _safe_image_path(relative_path)
    return _queue_public_artifact_path(relative_path)
...
def _retention_cleanup_targets(retention_days: int) -> list[tuple[str, int]]:
    ...
    for path in root.rglob("*"):
        ...
        if relative not in protected:
            targets.append((relative, stat.st_size))
    return targets
...
def delete_images(paths: list[str] | None = None, start_date: str = "", end_date: str = "", all_matching: bool = False) -> dict[str, int]:
    ...
    protected = config.protected_image_paths()
    for item in targets:
        relative = str(item or "").replace("\\", "/").lstrip("/")
...
def compress_images(quality: int = 60) -> dict:
    ...
    for p in sorted(config.images_dir.rglob("*.png")):
        if not p.is_file():
            continue
        if p.resolve().relative_to(root).as_posix() in protected:
            continue
        try:
            orig = p.stat().st_size
...
def delete_to_target(target_free_mb: int, dry_run: bool = False) -> dict:
    ...
    files = sorted(
        (
            ...
            if p.is_file() and p.resolve().relative_to(root).as_posix() not in protected
        ),
