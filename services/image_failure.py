...
class ImageFailure:
    ...
    def switch_account(self) -> bool:
        return self.outcome == "failure" and self.scope in {"account", "transient"}
...
FAILURE_POLICIES: dict[str, FailurePolicy] = {
    ...
    "image_task_pending": FailurePolicy(
        "internal", "image_generation", True, 504, "server_error",
    ),
    "image_queue_unavailable": FailurePolicy(
        "internal", "image_generation", True, 503, "server_error",
    ),
    "queue_timeout": FailurePolicy(
        "transient", "image_generation", False, 503, "server_error",
    ),
    ...
    "idempotency_conflict": FailurePolicy(
        "request", None, False, 409, "invalid_request_error",
    ),
    ...
    "image_queue_storage_full": FailurePolicy(
        "delivery", None, False, 507, "server_error",
    ),
    "quota_commit_failed": FailurePolicy(
        "internal", "image_generation", True, 503, "server_error",
    ),
...
class ImageGenerationError(ImageFailureError):
    ...
    def to_openai_error(self) -> dict[str, Any]:
        ...
        if self.task_id:
            payload["error"]["task_id"] = self.task_id
