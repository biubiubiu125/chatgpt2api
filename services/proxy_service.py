...
class ProxySettingsStore:
    ...
    def build_headers(
        ...
        if _find_header_key(merged_headers, "user-agent") is None:
            merged_headers["User-Agent"] = DEFAULT_PROXY_RUNTIME_USER_AGENT
...
def test_proxy(url: str = "", *, timeout: float = 15.0) -> dict:
    ...
    session = Session(impersonate="chrome146", verify=not proxy_settings.should_skip_ssl_verify(), proxy=candidate)
    started = time.perf_counter()
    try:
        response = session.get(
            "https://chatgpt.com/api/auth/csrf",
            headers={"user-agent": DEFAULT_PROXY_RUNTIME_USER_AGENT},
            timeout=timeout,
        )
