...
def canonical_body(
    ...
    *,
    ...
    cache_identity = str(body.get("_cache_identity") or "").strip()
    if cache_identity:
        payload["_cache_identity"] = cache_identity
    payload["messages"] = messages
    payload["stream"] = bool(stream)
    payload["protocol"] = str(protocol or "chat")
    return payload
...
def cache_key(
    ...
    *,
    ...
    encoded = json.dumps(
        _json_safe(canonical_body(body, messages, stream=stream, protocol=protocol)),
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
...
def normalize_text_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not settings.get("enabled") or not settings.get("normalize_messages"):
        return messages
