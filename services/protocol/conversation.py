...
from services.image_queue.types import JobCheckpoint, JobStage
...
from utils.helper import (
    ...
    split_image_model,
    validate_message_image_collection,
)
...
def _check_image_cancellation(request: "ConversationRequest") -> None:
    callback = request.cancel_requested_callback
...
def _resolve_image_urls_with_monitor(
    ...
    if request.cancel_requested_callback and "cancel_callback" not in kwargs:
        kwargs["cancel_callback"] = request.cancel_requested_callback
...
def normalize_messages(messages: object, system: Any = None) -> list[dict[str, Any]]:
    ...
    if isinstance(messages, list):
        for message in messages:
            ...
            if images:
                image_count, image_bytes = validate_message_image_collection(
                    images,
...
class ConversationRequest:
    ...
    managed_access_token: str = ""
    managed_account_id: str = ""
    checkpoint_callback: Any = None
    cancel_requested_callback: Any = None
...
def _recover_after_image_stream_timeout(
    ...
    if request.checkpoint_callback and (conversation_id or file_ids or sediment_ids):
...
def _image_result_output_from_urls(
    ...
    if request.image_result_formatter:
        formatted_items = [
            request.image_result_formatter(image_data, {
                "conversation_id": conversation_id,
                "image_urls": list(image_urls),
                "index": item_index,
            ...
            for item_index, image_data in enumerate(downloaded_images)
        ]
        formatted = {
            "created": int(time.time()),
            "data": [item for item in formatted_items if isinstance(item, dict)],
            "_image_urls": [
                str(item.get("url") or "")
...
def stream_image_outputs(
    ...
    _check_image_cancellation(request)
    last: dict[str, Any] = {}
    persisted_checkpoint: tuple[str, tuple[str, ...], tuple[str, ...]] | None = None

    def persist_remote_checkpoint(event: Mapping[str, Any]) -> None:
        nonlocal persisted_checkpoint
        if request.checkpoint_callback is None:
            return
        conversation_id = str(event.get("conversation_id") or "")
        file_ids = tuple(str(item) for item in event.get("file_ids") or [])
        sediment_ids = tuple(str(item) for item in event.get("sediment_ids") or [])
        checkpoint_key = (conversation_id, file_ids, sediment_ids)
        if not any(checkpoint_key) or checkpoint_key == persisted_checkpoint:
            return
        request.checkpoint_callback(JobCheckpoint(
            stage=JobStage.RESOLVING,
            conversation_id=conversation_id,
    ...
    file_ids = [str(item) for item in last.get("file_ids") or []]
    sediment_ids = [str(item) for item in last.get("sediment_ids") or []]
    persist_remote_checkpoint(last)
    message = str(last.get("text") or "").strip()
    stream_failure = last.get("_image_failure")
    stream_failure = stream_failure if isinstance(stream_failure, ImageFailure) else None
...
def _generate_single_image(
    ...
    max_account_attempts = (
        1
        if request.managed_access_token
        else config.image_max_account_attempts
    ...
    while True:
        ...
        try:
            if request.managed_access_token:
                token = request.managed_access_token
                attempted_tokens.add(token)
        ...
        try:
            ...
            backend.cancel_callback = request.cancel_requested_callback
            if request.trace_image_perf:
            ...
            if not request.defer_conversation_cleanup:
...
def cleanup_managed_image_conversation(access_token: str, conversation_id: str) -> None:
    token = str(access_token or "").strip()
    target = str(conversation_id or "").strip()
    if not token or not target:
        return
    with OpenAIBackendAPI(token) as backend:
        backend.delete_conversation(target)


def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    """Managed single-image helper used by the durable queue job path.

    External image APIs must enter the PostgreSQL durable queue. This function
    no longer creates per-request thread pools for public multi-image traffic.
    """
    if not request.managed_access_token:
        _monitor_image_stage(
            request,
            "image_local_rejected",
            local_reason="durable_context_required",
            status="failed",
        )
        raise ImageGenerationError(
            ...
            failure=image_failure(
                ...
                raw_detail="use the PostgreSQL image task path instead of the legacy thread pool",
            ),
        )
    ...
    # Durable jobs always run as n=1; multi-image is split at the queue layer.
    managed = replace(request, n=1)
    outputs = _generate_single_image(managed, 1, 1)
    if not outputs:
        raise ImageGenerationError(
...
def stream_image_chunks(
    ...
    for output in outputs:
        if output.kind == "progress" and output.task_id:
            status = (
                output.upstream_event_type
                if output.upstream_event_type in {"queued", "in_progress"}
                else "in_progress"
            )
            ...
            for item_index, item in enumerate(output.data):
                ...
                if b64_json:
                    completed["b64_json"] = b64_json
                for field in ("width", "height", "revised_prompt", "url"):
                    if item.get(field) not in (None, ""):
                        completed[field] = item[field]
                if usage_builder:
