...
from services.image_service import (
    ...
    get_image_response,
    get_local_image_response,
    get_local_thumbnail_response,
...
def _mask_config_secret(value: object) -> str:
    return SECRET_MASK if str(value or "").strip() else ""


def _public_settings(data: dict[str, object]) -> dict[str, object]:
    public = deepcopy(data)
    backup = public.get("backup")
    if isinstance(backup, dict):
        secret_access_key = str(backup.get("secret_access_key") or "").strip()
        passphrase = str(backup.get("passphrase") or "").strip()
        backup["secret_access_key"] = _mask_config_secret(secret_access_key)
        backup["passphrase"] = _mask_config_secret(passphrase)
        backup["has_secret_access_key"] = bool(secret_access_key)
        backup["has_passphrase"] = bool(passphrase)
    image_storage = public.get("image_storage")
    if isinstance(image_storage, dict):
        webdav_password = str(image_storage.get("webdav_password") or "").strip()
        image_storage["webdav_password"] = _mask_config_secret(webdav_password)
        image_storage["has_webdav_password"] = bool(webdav_password)
        private_webdav_password = str(image_storage.get("private_webdav_password") or "").strip()
        image_storage["private_webdav_password"] = _mask_config_secret(private_webdav_password)
        image_storage["has_private_webdav_password"] = bool(private_webdav_password)
    ai_review = public.get("ai_review")
    if isinstance(ai_review, dict):
        api_key = str(ai_review.get("api_key") or "").strip()
        ai_review["api_key"] = _mask_config_secret(api_key)
        ai_review["has_api_key"] = bool(api_key)
    basic = public.get("basic")
    if isinstance(basic, dict):
        api_key = str(basic.get("api_key") or "").strip()
        basic["api_key"] = _mask_config_secret(api_key)
        basic["has_api_key"] = bool(api_key)
    return public
...
@lru_cache(maxsize=2)
def _cluster_join_store(database_url: str) -> ClusterJoinStore:
    return ClusterJoinStore(database_url)


def _list_cluster_join_records() -> tuple[list[dict[str, Any]], str]:
    settings = load_cluster_settings()
    database_url = str(os.getenv("APP_DATABASE_URL") or "").strip()
    if not settings.is_api_main or not database_url:
        return [], ""
    try:
        return _cluster_join_store(database_url).list_worker_joins(), ""
    except Exception as exc:
        logger.warning({
            "event": "cluster_join_state_read_failed",
            "error": str(exc),
        })
        return [], str(exc)[:300]
...
def create_worker_public_router(app_version: str) -> APIRouter:
    router = APIRouter()

    @router.get("/images/{image_path:path}", include_in_schema=False)
    async def get_image(image_path: str):
        return await run_in_threadpool(get_local_image_response, image_path)

    @router.get("/image-thumbnails/{image_path:path}", include_in_schema=False)
    async def get_image_thumbnail(image_path: str):
        return await run_in_threadpool(get_local_thumbnail_response, image_path)

    @router.get("/health/live", response_model=None)
    async def health_liveness(format: str = Query(default="json")):
        payload = {
            "status": "ok",
            "healthy": True,
            "version": app_version,
    ...
    async def health_worker(
        ...
        scope: str = Query(default="runtime", pattern="^(business|runtime)$"),
    ):
        queue_health = image_task_service.health_summary()
        healthy = bool(queue_health.get("healthy", False))
        public_queue_health = {
            "status": str(queue_health.get("status") or ("ok" if healthy else "unavailable")),
            "healthy": healthy,
        }
        payload = {
            ...
            "scope": scope,
            "node_role": "worker",
            "image_queue": public_queue_health,
        }
        if format == "json":
            return JSONResponse(payload, status_code=200 if healthy else 503)
        return HTMLResponse(
            f"<html><body><h1>{'运行正常' if healthy else '运行降级'}</h1>"
            f"<p>worker</p></body></html>",
...
def create_router(app_version: str) -> APIRouter:
    ...
    async def get_settings(authorization: str | None = Header(default=None)):
        ...
        return {"config": _public_settings(config.get())}
    ...
    async def save_settings(body: SettingsUpdateRequest, authorization: str | None = Header(default=None)):
        ...
        try:
            ...
            if not updates:
                return {"config": _public_settings(config.get())}
            return {"config": _public_settings(config.update(updates))}
    ...
    @router.get("/images/{image_path:path}", include_in_schema=False)
    async def get_image(image_path: str):
        if load_cluster_settings().node_role == ROLE_API_MAIN:
            raise HTTPException(status_code=404, detail="images are delivered by their worker node")
    ...
    async def get_dashboard(
        ...
        image_queue_health = image_task_service.health_summary()
        healthy = bool(account_healthy and image_queue_health.get("healthy", False))
        storage = config.get_storage_backend()
        ...
        return {
            ...
            "image_queue": image_queue_health,
            "storage": {
                "backend": storage.get_backend_info(),
    ...
    async def get_cluster_state(authorization: str | None = Header(default=None)):
        require_admin(authorization)
        node = current_node_payload()
        queue_snapshot: dict[str, Any] = {}
        if getattr(image_task_service, "repository", None) is not None:
            queue_snapshot = await run_in_threadpool(image_task_service.repository.queue_snapshot)
        accounts = await run_in_threadpool(account_service.get_stats)
        register_state = await run_in_threadpool(register_service.get)
        joins, join_error = await run_in_threadpool(_list_cluster_join_records)
...
<title>Liveness - chatgpt2api</title></head>
<body><div>chatgpt2api live v{app_version}</div></body></html>""")

    @router.get("/health", response_model=None)
    async def health_dashboard(
        format: str = Query(default="html"),
        scope: str = Query(default="business", pattern="^(business|runtime)$"),
        authorization: str | None = Header(default=None),
    ):
        from services.account_service import account_service as acct_svc

        stats = acct_svc.get_stats()
        account_healthy = stats["active"] > 0 or stats["unlimited_quota_count"] > 0
        image_queue_health = image_task_service.health_summary()
        image_queue_healthy = bool(image_queue_health.get("healthy", False))
        cluster_runtime_health = build_cluster_runtime_health(current_node_payload(), [])
        if load_cluster_settings().is_api_main:
            try:
                queue_snapshot: dict[str, Any] = {}
                if getattr(image_task_service, "repository", None) is not None:
                    queue_snapshot = await run_in_threadpool(image_task_service.repository.queue_snapshot)
                ...
                cluster_runtime_health = dict(cluster_snapshot.get("runtime_health") or cluster_runtime_health)
            except Exception as exc:
                cluster_runtime_health = {
                    "required": True,
                    "healthy": False,
                    "status": "degraded",
                    "error": str(exc)[:300],
        ...
        image_queue_healthy = bool(image_queue_healthy and cluster_runtime_health.get("healthy", True))
        runtime_scope = scope == "runtime"
        healthy = bool(image_queue_healthy if runtime_scope else account_healthy and image_queue_healthy)
        ...
        if authorization:
            ...
            storage = config.get_storage_backend()
            payload.update({
                "cluster_runtime": cluster_runtime_health,
...
<html lang="zh">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>健康检查 - chatgpt2api</title>
<style>
body{{font-family:system-ui,-apple-system,sans-serif;background:#0f1117;color:#e2e8f0;min-height:100vh;margin:0;display:flex;align-items:center;justify-content:center}}
.panel{{max-width:560px;width:100%;margin:24px;background:#1a1d27;border:1px solid #2a2d3a;border-radius:10px;padding:24px}}
...
.api-url{{font-family:monospace;color:#6c63ff}}
</style>
</head>
<body>
<div class="panel">
<div class="status {'ok' if healthy else 'bad'}">{'运行正常' if healthy else '运行降级'}</div>
<div class="meta">版本 v{app_version}</div>
<div class="meta">JSON 接口 <span class="api-url">/health?format=json</span></div>
</div>
</body></html>""")
