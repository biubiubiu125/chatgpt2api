...
MAX_JSON_IMAGE_TOTAL_BYTES = 50 * 1024 * 1024
...
def _decode_json_image_string(value: str, index: int, filename: str | None = None, mime_type: str | None = None) -> tuple[bytes, str, str]:
    ...
    if match:
        ...
        encoded = text
    if len(encoded) > 4 * ((MAX_JSON_IMAGE_BYTES + 2) // 3):
...
def normalize_json_edit_images(image: object = None, images: object = None) -> list[tuple[bytes, str, str]]:
    ...
    validate_message_image_collection([(data, mime) for data, _, mime in normalized])
    return normalized


def validate_message_image_collection(
    images: list[tuple[bytes, str]],
    *,
    existing_count: int = 0,
    existing_bytes: int = 0,
) -> tuple[int, int]:
    count = existing_count + len(images)
    if count > MAX_JSON_EDIT_IMAGES:
        raise HTTPException(
            status_code=400,
            detail={"error": f"images supports up to {MAX_JSON_EDIT_IMAGES} items"},
        )
    for data, _ in images:
        if len(data) > MAX_JSON_IMAGE_BYTES:
            raise HTTPException(status_code=400, detail={"error": "image file is too large"})
    total_bytes = existing_bytes + sum(len(data) for data, _ in images)
...
def is_non_public_image_model(model: object) -> bool:
    ...
    if normalized in IMAGE_MODELS:
        return True
    return (
        normalized.startswith(("dall-e", "dalle", "imagen"))
...
def _stream_error_payload(
    ...
    if isinstance(exc, UpstreamHTTPError):
        ...
        if exc.retry_after is not None:
            payload["retry_after"] = int(exc.retry_after)
        return payload
...
def _decode_message_image_url(value: object) -> tuple[bytes, str] | None:
    ...
    try:
        ...
        image_data, _, mime = _download_image_url(source, max_bytes=MAX_JSON_IMAGE_BYTES)
...
def extract_image_from_message_content(content: object) -> list[tuple[bytes, str]]:
    ...
    image_items = [
        item
        for item in content
        if isinstance(item, dict) and str(item.get("type") or "").strip() in {"image_url", "input_image", "image"}
    ]
    if len(image_items) > MAX_JSON_EDIT_IMAGES:
        raise HTTPException(
            status_code=400,
            detail={"error": f"images supports up to {MAX_JSON_EDIT_IMAGES} items"},
        )
    ...
    for item in image_items:
        ...
        if item_type == "image_url":
            ...
            if image:
                images.append(image)
    validate_message_image_collection(images)
    return images
