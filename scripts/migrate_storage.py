...
from services.storage.factory import create_storage_backend, is_sqlite_database_url


def _split_backup_payload(payload):
    if isinstance(payload, list):
        return payload, []
    if not isinstance(payload, dict):
        raise ValueError("Invalid JSON format, expected object or account array")
    accounts = payload.get("accounts")
    auth_keys = payload.get("auth_keys", [])
    if not isinstance(accounts, list):
        raise ValueError("Invalid JSON format, expected accounts array")
    if not isinstance(auth_keys, list):
        raise ValueError("Invalid JSON format, expected auth_keys array")
    return accounts, auth_keys
...
def export_to_json(output_file: str):
    ...
    print(f"[migrate] Exported {len(accounts)} accounts and {len(auth_keys)} auth keys to {output_file}")
...
def import_from_json(input_file: str):
    ...
    try:
        ...
        accounts, auth_keys = _split_backup_payload(payload)
    ...
    print(f"[migrate] Imported {len(accounts)} accounts and {len(auth_keys)} auth keys")
...
def migrate_data(from_backend: str, to_backend: str):
    ...
    database_url_was_set = "DATABASE_URL" in os.environ
    ...
    try:
        ...
        print(f"[migrate] Loaded {len(accounts)} accounts and {len(auth_keys)} auth keys from {from_backend}")
        
        # 写入目标后端
        os.environ["STORAGE_BACKEND"] = to_backend
        if (
            to_backend.strip().lower() == "sqlite"
            and os.environ.get("DATABASE_URL", "").strip()
            and not is_sqlite_database_url(os.environ["DATABASE_URL"].strip())
        ):
            os.environ.pop("DATABASE_URL", None)
        to_storage = create_storage_backend(DATA_DIR)
        to_storage.save_accounts(accounts)
        to_storage.save_auth_keys(auth_keys)
        print(f"[migrate] Saved {len(accounts)} accounts and {len(auth_keys)} auth keys to {to_backend}")
