...
def _chrome146_user_agent(value: object = "") -> str:
    ...
    return user_agent if "Chrome/146" in user_agent else DEFAULT_USER_AGENT
    ...
    runtime["clearance"].update(
        {
            ...
            "mode": clearance_mode,
            "user_agent": _chrome146_user_agent(os.getenv("CHATGPT2API_PROXY_RUNTIME_USER_AGENT", DEFAULT_USER_AGENT)),
...
def main() -> int:
    ...
    if not config_path.exists():
        ...
        print(f"Config path is a directory, expected a JSON file: {config_path}", file=sys.stderr)
        return 1
    else:
        try:
    ...
    if (
        ...
        if isinstance(clearance, dict):
            normalized_user_agent = _chrome146_user_agent(clearance.get("user_agent"))
            if clearance.get("user_agent") != normalized_user_agent:
                clearance["user_agent"] = normalized_user_agent
                changed = True
